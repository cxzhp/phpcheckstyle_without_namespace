...
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Config/CheckStyleConfig.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Config/CheckArrayStyleConfig.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Config/CheckXMLStyleConfig.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/Reporters.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/Reporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/ArrayReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/ConsoleReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/HTMLConsoleFormatReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/HTMLFormatReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/PlainFormatReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/XmlConsoleFormatReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/XmlFormatReporter.php");
//include_once(PHPCHECKSTYLE_HOME_DIR."/src/PHPCheckstyle/Reporter/XmlNCSSReporter.php");
...
class PHPCheckstyle {
	...
	private $_extendsClassName = null;
	...
	private function _processToken($token) {
		...
		switch ($token->id) {
			...
			case T_STRING:
				if(TOKEN_SELF == $token->text){
					$this->_checkSelfToken($token);
				}
...
$this->tokenizer->checkNextToken(T_DNUMBER))) {
	...
	private function _processFunctionCall($text) {
		...
		//invalidTiebaServiceCall
		if('tieba_service' == strtolower($text)){
			if('molib_client_baseaction' == strtolower($this->_extendsClassName)){

				/*
				 * can not use Tieba_Service with Molib_Client_BaseAction
				 */
				if ($this->_isActive('invalidTiebaServiceCall') ) {
					$msg = $this->_getMessage('INVALID_TIEBA_SERVICE_CALL' );
					$this->_writeError('invalidTiebaServiceCall', $msg);
				}
		...
		if('notice' == strtolower($text)){
			...
			if ($previousToken->position > 1) {
				...
				if('bingo_log' == strtolower($prepreviousToken->text)){
					/*
					 * can not use Bingo_Log::Notice()
					 */
					if ($this->_isActive('invalidNoticeLog') ) {
						$msg = $this->_getMessage('INVALID_NOTICE_LOG' );
						$this->_writeError('invalidNoticeLog', $msg);
					}
				}
			}
	...
	/**
	 * Process the extendsClassName declaration.
	 */
	private function _processExtendsClassName() {
		// We replace the extendsClassName name with the extends
		$this->_extendsClassName = $this->tokenizer->peekNextValidToken()->text;
	}
